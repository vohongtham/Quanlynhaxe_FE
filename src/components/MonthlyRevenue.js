...
const MonthlyRevenue = () => {
    const [yearMonth, setYearMonth] = useState(''); // State to hold the YYYY-MM value
    const [revenueData, setRevenueData] = useState(null); // Store revenue data
    const [loading, setLoading] = useState(false); // Loading state
    const [displayMonthYear, setDisplayMonthYear] = useState(''); // State to hold formatted month/year for display
    ...
    // Function to fetch revenue by month and year for a specific parking lot
    const fetchMonthlyRevenue = async () => {
        if (yearMonth && selectedParkingLot) {
            const [year, month] = yearMonth.split('-'); // Split into year and month
            setLoading(true);
            try {
                const result = await StatisticalService.getByMonth(month, year, selectedParkingLot); // Updated to include selectedParkingLot
                setRevenueData(result); // Store the revenue data in state
                setDisplayMonthYear(formatYearMonth(yearMonth)); // Update the displayed month/year
            } catch (error) {
                toast.error(`Error fetching monthly revenue: ${error.message}`);
            } finally {
                setLoading(false);
            }
        } else {
            toast.warn('Chọn tháng và nhà xe cần thống kê.');
        }
    };

    // Handle input change for <input type="month">
    const handleYearMonthChange = (e) => {
        const value = e.target.value; // This will give YYYY-MM format
        setYearMonth(value); // Set the value in state
    };
    ...
    // Format yearMonth to display as YYYY/MM
    const formatYearMonth = (value) => {
        return value.replace('-', '/'); // Convert YYYY-MM to YYYY/MM
    };
    ...
    //             'Tháng thống kê': displayMonthYear,
    ...
    //     // Xuất file Excel
    //     XLSX.writeFile(workbook, `DoanhThu_${selectedParkingLot}_${yearMonth}.xlsx`);
    ...
    const saveToPDF = () => {
        ...
        doc.text('Thong ke thang: ' + displayMonthYear, 14, 70);
        ...
        // Save PDF
        doc.save('DoanhThu_' + selectedParkingLot + '_' + displayMonthYear + '.pdf');
    ...
    return (
        <div >
            <h3 className="text-center">THỐNG KÊ THEO THÁNG</h3>
            <div style={{ marginLeft: '180px', fontSize: '20px'}}>
                <label htmlFor="yearMonth" >Chọn tháng thống kê: </label>
                <input
                    type="month"
                    id="yearMonth"
                    value={yearMonth}
            ...
            <div style={{ marginLeft: '180px', fontSize: '20px'}}>
                <label htmlFor="parkingLot">Chọn nhà xe: </label>
                <select
                    id="parkingLot"
                    ...
                    style={{ 
                        ...
                        padding: '5px', 
                        borderRadius: '5px', 
                        border: '3px solid #ccc' }} // Adjust width and margin
                >
                ...
                <button className="btn btn-sm btn-primary" onClick={fetchMonthlyRevenue} disabled={loading}
                    style={{ fontSize: '18px'}}    
                >
            ...
            <div className="mt-3" style={{ fontSize: '18px', marginLeft: '50px', display: 'flex', gap: '50px' }}>
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                        <p style={{ marginRight: '10px' }}><strong>Tháng thống kê:</strong></p>
                        <input
                            type="text"
                            value={displayMonthYear}
                            readOnly
                            style={{ width: '300px', marginLeft: '42px', borderRadius: '2px' }}
                        />
                    </div>
