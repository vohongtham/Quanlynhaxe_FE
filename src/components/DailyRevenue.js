...
const DailyRevenue = () => {
    ...
    const [displayDate, setDisplayDate] = useState(''); // State to hold formatted date for display
    ...
    // Function to fetch revenue by selected date for a specific parking lot
    const fetchDailyRevenue = async () => {
        if (selectedDate && selectedParkingLot) {
            setLoading(true);
            try {
                const result = await StatisticalService.getByDay(selectedDate, selectedParkingLot); // Call the API
                console.log('Revenue data received from server:', result); // Log the result from the server
                setRevenueData(result); // Store the revenue data in state
                setDisplayDate(formatDate(selectedDate)); // Update the displayed date
            } catch (error) {
                toast.error(`Error fetching daily revenue: ${error.message}`);
            } finally {
                setLoading(false);
            }
        } else {
            toast.warn('Chọn ngày và nhà xe cần thống kê.');
        }
    };

    // Handle input change for <input type="date">
    const handleDateChange = (e) => {
        const value = e.target.value; // This will give YYYY-MM-DD format
        setSelectedDate(value); // Set the value in state
    };
    ...
    //     // Xuất file Excel
    //     XLSX.writeFile(workbook, `DoanhThu_${selectedParkingLot}_${selectedDate}.xlsx`);
    ...
    const saveToPDF = () => {
        ...
        doc.text('Thong ke ngay: ' + displayDate, 14, 70);
        ...
        // Save PDF
        doc.save('DoanhThu_' + selectedParkingLot + '_' + displayDate + '.pdf');
    ...
    return (
        <div>
            <h3 className="text-center">THỐNG KÊ THEO NGÀY</h3>
            <div style={{ marginLeft: '180px', fontSize: '20px'}}>
                <label htmlFor="selectedDate">Chọn ngày thống kê: </label>
                <input
                    type="date"
                    id="selectedDate"
                    value={selectedDate}
            ...
            <div style={{ marginLeft: '180px', fontSize: '20px'}}>
                ...
                <button className="btn btn-sm btn-primary" onClick={fetchDailyRevenue} disabled={loading} 
                    style={{ fontSize: '18px'}}
                >
            ...
            <div className="mt-3" style={{ fontSize: '18px', marginLeft: '50px', display: 'flex', gap: '50px' }}>
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                        <p style={{ marginRight: '10px' }}><strong>Ngày thống kê:</strong></p>
                        <input
                            type="text"
                            value={displayDate}
                            readOnly
                            style={{ width: '300px', marginLeft: '50px', borderRadius: '2px' }}
                        />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
